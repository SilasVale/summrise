...
| `scripts/test/landing-contrast-check.mjs` | lighten a label the landing paints (the tertiary `#71717a` → `#9a9aa2`), or take its heading away (the `<h1>` back to a `<div>`) | exit 1 for the colour: "2.68 on #fafafa, under the 4.5 AA wants for text"; exit 1 for the document: "expected exactly 1 h1, found 0". Its values are read FROM page.js, so a rename there fails the token floor instead of quietly checking nothing. Its DARK pairs are not covered yet — the page has a theme toggle and the measurements are the light ones |
