...
| `scripts/test/landing-contrast-check.mjs` | lighten a label the landing paints (the tertiary `#71717a` → `#9a9aa2`) | exit 1: "2.68 on #fafafa, under the 4.5 AA wants for text". Its values are read FROM page.js, so a rename there fails the token floor instead of quietly checking nothing |
