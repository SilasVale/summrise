...
test("tools/call without a device arg resolves the single registered device", async () => {
  const env = makeEnv([{ name: "d1", hostname: "d1.agent.summrise.test", token: "devtok" }]);
  ...
  try {
    ...
    assert.ok(paths.some((u) => u.includes("d1.agent.summrise.test/api/tools/terminal_list")), "must proxy to the resolved device");
...
test("secret_get routes to the DEVICE agent, not the browser extension", async () => {
  ...
  try {
    ...
    assert.ok(hits.some((u) => u.includes("d1.agent.summrise.test/api/tools/secret_get")), "must fetch from the device agent");
