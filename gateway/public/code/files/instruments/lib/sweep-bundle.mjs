 ...
 */
/**
 * THE PIECES MODULE, GENERATED IN ONE PLACE (round 272). Four emitters had grown their own `piecesSource()`, each with
 * the same two rules — a FUNCTION crosses as code (`fn.toString()`), everything else as JSON — copied four times. The
 * review's criterion for this seam was "escaping owned once"; this is that. The one subtlety worth stating: a function
 * is declared at the TOP of the module and referenced by name, never inlined into the exports object, because the
 * passes CALL EACH OTHER by name (`pressPass` calls `discoverPressTargets`) and an inline function expression would
 * leave that name undefined at run time.
 *
 * @param {Record<string, unknown>} pieces  strings/numbers/objects are JSON; functions become declarations
 * @returns {string} the source of a `pieces.cjs`
 */
export function piecesModule(pieces) {
  const decls = [];
  const used = new Set();
  const expr = (value) => {
    if (typeof value === "function") {
      let name = value.name || "piece";
      while (used.has(name)) name += "_";
      used.add(name);
      decls.push(`const ${name} = ${value.toString()};`);
      return name;
    }
    if (value && typeof value === "object" && !Array.isArray(value)) {
      return `{ ${Object.entries(value)
        .map(([k, v]) => `${JSON.stringify(k)}: ${expr(v)}`)
        .join(", ")} }`;
    }
    return JSON.stringify(value);
  };
  const body = Object.entries(pieces)
    .map(([k, v]) => `  ${JSON.stringify(k)}: ${expr(v)}`)
    .join(",\n");
  return (
