 ...
 * OR the per-device summrise_pt_<name> cookie minted by the ?token= 302
...
export async function handleDeviceProxy(request: Request, env: any, url: URL): Promise<Response> {
  ...
  try {
    cookieToken = decodeURIComponent(
      parseCookie(request.headers.get("cookie") || "")[`summrise_pt_${deviceName}`] || "",
    );
  } catch {
    ...
    if (qToken && isNav) {
      ...
      return new Response(null, {
        ...
        headers: {
          ...
          "Set-Cookie": `summrise_pt_${deviceName}=${encodeURIComponent(qToken)}; Path=${DEVICE_BASE}/${deviceName}/proxy; HttpOnly; Secure; SameSite=Lax; Max-Age=2592000`,
...
async function proxyDevice(
  ...
  // request carries the shared proxy secret (X-Summrise-Auth) — the R102 marker
  ...
  // Strip inbound FIRST: a client-sent x-summrise-auth must never ride through
  // when the record has no proxySecret (the header is ours to mint).
  headers.delete("x-summrise-auth");
  if (device.proxySecret) headers.set("x-summrise-auth", device.proxySecret);
...
// Absolute paths a summrise-agent panel serves from its own root. When proxied
