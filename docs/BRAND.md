...
| GitHub releases | `v1.2.455` published with its CI-built asset, and the **dual-builder audit passed byte-for-byte** — `CDN == GitHub asset (584c1d4f…)`, the same sha256 the manifest records. `v1.2.454` did the same, exe included, which was the first release where it could: its tag sits on the commit the pack was built from. `v1.2.453`'s asset packages a LATER tree and the audit refuses it, correctly — see the ledger |
