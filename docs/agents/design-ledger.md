...
| the early round-numbered entries — **MOVED OUT in round 273**, read them in `docs/agents/ledger-early-rounds.md` | rounds 54 to 162, in the order they were written (descending): the loop's own mistakes and the fixes, from before the series was renamed to "the Nth exploration". They moved when this file crossed the 400,000-byte ceiling its own gate had stopped enforcing | `round 162 — round 161's four candidates dissolve under the rules themselves, and the extractor was wrong three ways` |
<!-- ledger-index:end -->

**Round 273 — one check, and it was DEAD CODE**: rounds 188 and 189 both concluded that this file has **no byte ceiling**, and
each said so in the note below. **The sentence is false, and the difference is three lines of control flow.**
`ledger-budget-check.mjs` has carried `ARCHIVE_CEILING = 400_000` and a check against it all along — but that check sat BELOW
`if (failures.length) { … process.exit(1) }`, so it pushed onto an array nothing ever read again. Measured before the fix, on an
unchanged tree: `node scripts/test/ledger-budget-check.mjs` printed `ok — … (457929 B of 400000)` and exited **0**, with this file
**57,929 bytes over its own limit**. Re-armed, the SAME tree exits **1**, naming the file and both numbers — which is the bite
proof, and the reason no mutation had to be planted for it. **The 134,110 bytes of round-numbered entries from round 162 down to
round 54 then moved to `docs/agents/ledger-early-rounds.md`** (a fourth archive, with its own floor and ceiling), and the
appendix — which had a floor and no ceiling while the file above it claims "a ceiling on each is what stops either growing back
into the other" — got the ceiling that sentence promised. **A CHECK PLACED AFTER THE ONLY CONSUMER OF ITS RESULT IS NOT A CHECK;
it is a message that always agrees** — and two rounds read that message and wrote down what it said. The pre-commit rule that
refuses an archive-only commit was widened from ONE file to all four in the same round, because leaving it naming only
`design-ledger.md` would have made the NEW archive the new loophole — the same failure, one file over.

**AND THREE THINGS THIS ROUND FOUND, ONLY ONE OF WHICH IT FIXED — which is the part worth writing down.** (1) **The title check
was reading QUOTED CODE as headings.** Nine `##`/`###` lines sitting inside fences across the four archives counted as titles a
section could resolve against, and the file's own AGENTS.md check applies the opposite rule twenty lines above ("headings inside
fenced code blocks are not headings"). FIXED — fences are stripped — and recorded as HARDENING rather than a fix, because the
measurement says no verdict changed: all nine index-named titles resolve either way, **293 headings unstripped and 284 stripped,
0 unresolved both times**. (2) **There is still no PER-SECTION ceiling**, which was round 188's real finding and is still unpaid:
the opening section of this file is **58,551 bytes — 17.9% of it — holding 21 `###` round entries nested under `## What is in
here`**, the same nesting round 188 fixed for 33 entries one level up. (3) **There is no real `## Committing` section in this file
...
that applies it had sat below the only consumer of its result since it was written, so it could not fail. See the round-273 note above;
the hook this paragraph describes is unaffected and still refuses the commit.]** The pre-commit hook now REFUSES a commit whose only staged change is this file, with a deliberate,
...
**[ROUND 273: HALF OF THIS IS STILL TRUE AND HALF NEVER WAS. There is still no PER-SECTION ceiling — that is owed and unpaid. But the
BYTE ceiling this paragraph implies was missing has existed all along and was dead code, so "the next round's work" was three lines of
control flow in a file nobody re-read. See the round-273 note above.]**
...
## Three more archives: `docs/agents/ledger-mutations.md`, `docs/agents/ledger-appendix.md`, `docs/agents/ledger-early-rounds.md`
...
`ledger-budget-check.mjs`, and the index above resolves a title against all four. The other half, which
...
**AND A FOURTH ARCHIVE ARRIVED IN ROUND 273, FOR THE REASON THIS PARAGRAPH WAS WRITTEN AND COULD NOT DELIVER.**
`docs/agents/ledger-early-rounds.md` holds the round-numbered entries from round 162 down to round 54 — 134,110 bytes of
narrative that had to go somewhere once the ceiling above this file started being enforced instead of printed. It is
narrative rather than a table, so it could not join the appendix (whose own header says its sections are LOOKUP TABLES); it
is the OLD end of this file's own kind, so it got a file of its own, with a floor and a ceiling like the rest. **A reader
following the index above never needs to know which of the four a title lives in — that is the property the title check
exists to keep true.**
