...
than trusting the number; a cell that was NOT re-measured in the 2026-09-24 pass says so where it sits. The
`agent/src` block was re-measured once more on 2026-09-25 at `main` = `279c9929`, and each of its cells names its
own SHA: a count is only reproducible at the SHA it was read from, which is the mistake these cells made
when a HEAD reading was written beside a previous commit's SHA.
...
| area | size | evidence |
|---|---|---|
| `agent/src` (Rust) | **54,889 lines** | RE-MEASURED AGAIN 2026-09-25 (round 71) at `main = bb88bbca`: the same command reads **54,889**, and `wc -l < agent/src/spawn.rs` → **1,693** — the round-70 gate work added fixtures for a third dead-on-Windows cfg spelling, which is why the file that implements the rule keeps growing while the rule itself stops needing waivers (EXEMPT is still zero). The earlier reading: RE-MEASURED 2026-09-25 (round 43): `git rev-parse --short HEAD` → **`9152fae6`**, and `find agent/src -name '*.rs' \| xargs wc -l` → **54,823** at that commit — **+483 over the 54,340 round 29 read**, and ALL of it is one file: `wc -l < agent/src/spawn.rs` → **1,627**, because round 42 taught the spawn gate's scan to read `#[cfg(unix)]` regions (with its own comment/string/raw-string state and four mutations) so that the waiver list could reach zero. A gate that reads source TEXT costs source LINES, and this cell is where that shows. The earlier reading, kept for contrast: 54,340 at `14c76409`. The cell read **51,915** at `279c9929` and the twelve rounds between them added **2,425 lines** — `spawn.rs` (the spawn policy), `transfer.rs` (one landing for a transfer), the live-config handle and their tests, which is the growth this re-measurement records. Earlier readings, kept for contrast and all carrying their own SHA: 51,915 at `279c9929`, 51,258 at `c321690f`, 51,127 at `f464bfd7`, 49,132 on 2026-09-24. The cell named `main = c321690f` beside this number and the two did not agree: the same command at `c321690f` reads **51,258** (the atomic-replace commit added 657 lines), so 51,915 is `279c9929`'s count. Earlier in the same pass it read 51,127 at `f464bfd7` and 49,132 on 2026-09-24 — the same drift every cell in this table has shown, which is why each now carries the date, the SHA and the command. (The review-fix pass that corrected this cell edits comments only and is uncommitted; the working tree reads 51,961.) |
| ├ plugins | 18,574 lines / 30 files | RE-MEASURED 2026-09-25 at `main = 14c76409` (round 29): `find agent/src/plugins -name '*.rs' \| xargs wc -l` → **18,574** and 30 files — +566 over the 18,008 this cell read at `279c9929`. The SHAs in the per-subdirectory readings below are the older pass's and are left as they stand; the TOTAL carries this one. The earlier pass, kept for contrast: `find agent/src/plugins -name '*.rs' \| xargs wc -l` → 18,008 and 30 files, the same command per subdirectory → terminal 5,957 · memory 3,915 · mcp_client 2,140 · update 1,908 · system 1,447 · playwright 1,133 · design 682 · **monitor 325** (was 213) · **runs 124** (was 134 — the retention round moved its window arithmetic out, so the module got smaller). The cell said "RE-MEASURED with the line above" over 17,983 and memory 3,890; both of those are the ROUND-27 tree (`f9dc564a`), not the SHA it pointed at (`c321690f` reads 17,991 and memory 3,898; `279c9929` reads 18,008 and 3,915) — the number and the SHA disagreed, which is what this re-measurement fixes. The 2026-09-24 column read terminal 5,771 · memory 3,869 · mcp_client 2,158 · update 1,576 · system 1,443 · playwright 1,123 · design 682 · monitor 213 · runs 134. (The working tree, after this review-fix pass's comment-only edits, reads 18,017 / memory 3,924.) |
| ├ tools (terminal backends) | **7,827 lines / 12 files** | RE-MEASURED 2026-09-25 at `main = 279c9929`: `find agent/src/tools -name '*.rs' \| xargs wc -l` → 7,827, and `find agent/src/tools -name '*.rs' \| wc -l` → 12. The cell said 7,677 "unchanged in the 2026-09-25 pass"; 7,677 is `c321690f`'s reading, and the atomic-replace commit moved this directory (+150: the multi-line `crate::atomic::replace` calls in `ssh.rs`, `connections.rs` and `secrets.rs`), so "unchanged" was the wrong half of the claim |
| └ web (HTTP + SSE + panel embed) | **9,671 lines / 4 files** | RE-MEASURED 2026-09-25: `find agent/src/web -name '*.rs' \| xargs wc -l` for the total and `ls agent/src/web/*.rs \| wc -l` for the count. It read 8,580 on 2026-09-24 — the route table, its tests and the whole-table envelope gate are most of the growth. (A first version of this edit said 5 files, having read the `wc` total line as a count; the same trap the panel cell below records, caught by running the counting command instead of the summing one). RE-MEASURED again 2026-09-25 (round 29) at `main = 14c76409`: the same command reads **9,671 / 4 files** — the same 9,665 the `279c9929` pass read plus six lines, and the six are the live-config handle's use in the token gate. The directory did not change shape; only that did. |
| MCP tools | **58** | `agent/spec-tools.json`, regenerated by `cargo test spec_snapshot` (`agent/src/web/mod.rs:2844`) |
| HTTP endpoints | **37 (method, path) shapes + `/mcp`** | RE-MEASURED 2026-09-24 (route-table round, `main = 7b9ddbd6`): the shapes now live in ONE table — `agent/src/web/mod.rs:898 routes()`, 42 rows = 24 `DispatchGated` + 6 `PreDispatchAuthed` + 2 `Mcp` + 10 `Public`, counted from the table itself. The former evidence for this cell (`24 dispatch arms :792-888`, `13 pre-dispatch shapes :595-670`) is GONE with the refactor: those line ranges were the two hand-written tables and the nested `dispatch`, which is now module scope at `:1514` and an exhaustive match over `route_of` (`:1180`). The cell's own numbers did not move — the refactor changed where the rows ARE, not what they are |
| plugins | **9**, all registered unconditionally (`agent/src/state.rs:80-100`) | no enable/disable flag exists |
| `summrise` CLI | 12 subcommands, **3,880 lines** | RE-MEASURED 2026-09-25 (round 29) at `main = 14c76409`: RE-MEASURED AGAIN 2026-09-25 (round 55) at `main = fd85e889`: `wc -l < agent/summrise-agent-npm/src/summrise.ts` → **3,880** and `wc -l < .../test/cli.test.mjs` → **2,891** (the round-54 shell work: eight call sites moved from a quoted shell string to argv, one to an environment reference, plus the comments that record the measurement and the pin that now checks quoting is not enough — 90 lines of CLI, 274 of test). Earlier readings: 3,790 / 2,617 at `957f0279`, 3,773 at `14c76409`, 3,651 before that.  at `main = 957f0279`: `wc -l < agent/summrise-agent-npm/src/summrise.ts` → **3,790**, and `wc -l < agent/summrise-agent-npm/test/cli.test.mjs` → **2,617** (a number this cell never carried: the file that holds the CLI's tests is now bigger than half the file it tests, which is what the shell-door round did — 17 lines of fix, 390 of pin). The earlier reading: 3,773 at `14c76409` (the component-address round, which made `resolveComponent` read `components.<name>.url` from the manifest it already fetched), and 3,651 before that, when this cell named the FILE and not the command. |
| panel SPA | **43,515 lines**; breakdown NOT re-measured — the 2026-09-24 counts (46 components · 19 hooks · 28 lib modules) are superseded by the 2026-09-25 measurement below | RE-MEASURED AGAIN 2026-09-25 (round 79) at `main = a9452590`: the same command reads **43,515** and the committed bundle is **663487 B** — the round-78 `read` option on the module plus `ConnModal` migrating onto it, with the suite at 890 tests. The earlier reading: RE-MEASURED AGAIN 2026-09-25 (round 74) at `main = 343aca8a`: the same command reads **43,205** — the round-73 read-seam work (a `keepEdits` option on the module, `SettingsPage` migrated onto it, five new tests) — and the committed bundle is **663146 B**, because `panel.js` is embedded with `include_str!` and moves with every source change. The panel suite is 882 tests at that commit. The earlier reading: RE-MEASURED AGAIN 2026-09-25 (round 57) at `main = 66a3c557`: the same command reads **42,843** (223 files) and `ls agent/resources/panel-react/src/hooks/*.ts | wc -l` → **20** — +125 lines over the 42,718 this cell read at `c321690f`, which is the ratchet round: `sessionFacts.test.ts` learned to guard COMMAND facts (`exitCode`, `reason`, `ended`) and `ActivityPage.tsx` stopped deciding for itself how a command ended. The committed bundle moved with it (`wc -c < agent/resources/panel/panel.js` → **662,473**), because panel.js is embedded with `include_str!` and is part of the diff rather than a by-product of it. The earlier reading, kept for contrast: RE-MEASURED 2026-09-25 (round 28, `main = 8e43f04c`): `find agent/resources/panel-react/src -type f \| xargs wc -l` → 42,718 on 2026-09-25 (round 28, `main = c321690f`; the same day it read 42,638, 42,305 earlier the same day, 41,641 the day before, 40,674 when this cell was written — the read migration and its tests are most of the growth). `ls agent/resources/panel-react/src/hooks/*.ts \| wc -l` → **20** (was 19): the migration added `useDeviceRead.ts`, a hook the old cell never counted. The sub-counts stay MARKED because the cell still does not state the definition that produced them; the totals carry their command. (A first version of THIS edit said 42,481 — typed from the previous reading rather than from the command's output. The number in a cell is the one thing a reader cannot check by re-reading, which is why the command sits beside it.) |
...
| docs | each file's size is `wc -l`'s answer, and there were **THREE archives when this cell was written**: `wc -l` read **design-ledger 4,286 · ledger-appendix 3,849 · ledger-mutations 850** (rounds 107-112). **[LANDING 4a: `design-ledger.md` IS DELETED — the round-narrative process was retired by the operator, and this cell is kept as the measurement it was, with the file it measured named as gone. The count is now two.]** The cell used to say the ledger "read 2,406 here and is over 4,600" — TRUE when written and FALSE after round 107 moved the 91 KB mutation table into its own file, which is the drift this cell exists to warn about and a reminder that a size in prose is a measurement with a date, not a property of the file | `wc -l docs/agents/design-ledger.md docs/agents/ledger-appendix.md docs/agents/ledger-mutations.md` |
...
| **the 15 MCP tools with no in-repo caller** | **not dead**: they are reachable through the device's own `/mcp` endpoint (`ConnectCard.tsx:119`), which is how external AI clients use them, and some carry an explicit `NOT_EXPOSED` reason in the gateway — **run `cd gateway && npm test` and read the `contract: NOT_EXPOSED contains no ghost and every entry is justified` line for the count**, because this cell cited `mcp-handler.test.mjs:260-296` and the list had MOVED to `:311-345`. The `13 of the 15` it also claimed is UNMEASURED, and the two are different sets: `NOT_EXPOSED` is what the CONSOLE withholds, while the `15` is about in-repo CALLERS. Per §1, a number that moves belongs to the suite that prints it. See §6.1 for the one question this raises |
...
|---|---|---|---|
| 4.1 | `GET /api/events` (SSE) | **RE-VERIFIED 2026-09-25 (round 77) and the claim HOLDS, PRECISELY**: the built `panel.js` has exactly ONE `api/events` match and it is **`api/events/term`** — a DIFFERENT, live route (`useSSE.ts:232`); the source mentions bare `/api/events` only inside a comment recording its removal (`useAiActivityPulse.ts:3-4`). The agent serves three routes, not one: `mod.rs:13-15` lists `/api/events`, `/api/events/poll` and `/api/events/term` | an out-of-repo client (older panel build, a phone shortcut) would break — see §6.4, **which is why this is the operator's call and not a round's: the measurement says nobody IN HERE uses it, and only the operator knows whether anybody outside does** |
...
| | **FIXED — verified 2026-09-24.** §8 records it: `stateFromEnd` is THE derivation in `lib/path.ts`, and `TrajectoryView.eventDotState` is gone. | |
...
| | **FIXED — verified 2026-09-24.** §8 records it: the desktop strip uses the disambiguated label now, as the panel has since round 167, with a mutation pinning it. | |
...
| | **FIXED — verified 2026-09-24, AND THE CITATION WAS DEAD.** `ci.yml:414-420` holds no pwsh content at all; the fix is at `ci.yml:522`, "A MISSING pwsh IS A FAILURE, NOT A WARNING". | |
...
| | **FIXED — verified 2026-09-24.** §8 records it: the test snapshots the tree and compares instead of requiring a clean checkout. Its own comment at `:49` still describes the old behaviour. | |
| 5.8 | ~~Seven e2e sections never run in CI~~ **CLOSED IN PRACTICE (rounds 82-116), and this cell was stale in the flattering direction**: CI still runs only two sections (`ci.yml:185-188`), and that is still by design — but the missing half was never a CI job, it was a CADENCE, and it now exists. `AGENTS.md` names the sections, the trigger, and the command; the script reaches the device over the CDN (published beside the panel build, fetched with an outbound GET, no listener — round 85, verified from the device at 54,783 B; the exact URL is in the ledger and in AGENTS.md, not here, because this file must not name a production host — `production-host-check` says so and caught it); and the sections have been RUN: **73/74 passed (2 skipped)** across all seven (round 116), with **two real defects found and fixed** by doing it — a panel selector that clicked a navigation tab instead of the newest session (`1/2` → `2/2`, round 90) and a terminal check demanding `state === 'done'` where the device returns `state=partial` every time (`11/12` → `12/12` twice, rounds 113-115) — plus one check that could pass vacuously and one SKIP state the suite did not have. **The one remaining failure is bounded rather than vague**: `mcp stdio click drives embedded view` fails under FULL-SUITE load and passes in section-only runs, which is what that check's own comment predicts | `[measured]`, and the numbers come from the device runs recorded in the ledger under rounds 87-116 |
| | **RE-VERIFIED 2026-09-24 and still correct, against the exploration that flagged it**: `e2e.js:47` declares NINE sections and `ci.yml:275` runs `--only governance,runs` — two of nine, so seven never run. | |
...
| verified by measurement on BOTH ends | the CI design job’s own line — `bash scripts/design-sweep-ci.bash`, which reports how many surfaces it rendered and how many text nodes it read (the sweep prints "48 surfaces" for the panel today; the HARNESS pages are the job’s other half) **and** `live-panel-probe.mjs` against the panel the device actually runs. **This cell used to say "142 harness surfaces in CI" — a number no instrument in this repository produces.** |
