...
| area | size | evidence |
|---|---|---|
| `agent/src` (Rust) | **51,915 lines** | RE-MEASURED 2026-09-25 (round 28, `main = c321690f`): `find agent/src -name '*.rs' \| xargs wc -l` → 51,915. The cell above it in this pass read 51,127 at `f464bfd7` and 49,132 on 2026-09-24 — the same drift every cell in this table has shown, which is why each now carries the date, the SHA and the command |
| ├ plugins | 17,983 lines / 30 files | RE-MEASURED with the line above: terminal 5,957 · memory 3,890 · mcp_client 2,140 · update 1,908 · system 1,447 · playwright 1,133 · design 682 · **monitor 325** (was 213) · **runs 124** (was 134 — the retention round moved its window arithmetic out, so the module got smaller). The 2026-09-24 column read terminal 5,771 · memory 3,869 · mcp_client 2,158 · update 1,576 · system 1,443 · playwright 1,123 · design 682 · monitor 213 · runs 134 |
| ├ tools (terminal backends) | 7,677 lines / 12 files | `agent/src/tools/` — unchanged in the 2026-09-25 pass |
| └ web (HTTP + SSE + panel embed) | **9,665 lines / 4 files** | RE-MEASURED 2026-09-25: `find agent/src/web -name '*.rs' \| xargs wc -l` for the total and `ls agent/src/web/*.rs \| wc -l` for the count. It read 8,580 on 2026-09-24 — the route table, its tests and the whole-table envelope gate are most of the growth. (A first version of this edit said 5 files, having read the `wc` total line as a count; the same trap the panel cell below records, caught by running the counting command instead of the summing one) |
