...
| `summrise` CLI | 12 subcommands, **3,790 lines** | RE-MEASURED 2026-09-25 (round 29) at `main = 14c76409`: RE-MEASURED AGAIN 2026-09-25 (round 38) at `main = 957f0279`: `wc -l < agent/summrise-agent-npm/src/summrise.ts` → **3,790**, and `wc -l < agent/summrise-agent-npm/test/cli.test.mjs` → **2,617** (a number this cell never carried: the file that holds the CLI's tests is now bigger than half the file it tests, which is what the shell-door round did — 17 lines of fix, 390 of pin). The earlier reading: 3,773 at `14c76409` (the component-address round, which made `resolveComponent` read `components.<name>.url` from the manifest it already fetched), and 3,651 before that, when this cell named the FILE and not the command. |
