...
| └ web (HTTP + SSE + panel embed) | **9,665 lines / 4 files** | RE-MEASURED 2026-09-25: `find agent/src/web -name '*.rs' \| xargs wc -l` for the total and `ls agent/src/web/*.rs \| wc -l` for the count. It read 8,580 on 2026-09-24 — the route table, its tests and the whole-table envelope gate are most of the growth. (A first version of this edit said 5 files, having read the `wc` total line as a count; the same trap the panel cell below records, caught by running the counting command instead of the summing one). Re-read at the SHA above (`279c9929`): still 9,665 / 4 files — the atomic-replace commit did not touch this directory |
