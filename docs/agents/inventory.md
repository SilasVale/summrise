...
| all four surfaces' gates and suites | the design job + panel 814/103 + gateway 921/0 + the Rust suite (three configurations, clippy included) + every gate command in `ci.yml` via `scripts/test/all-gates.bash`, and the console's render smokes via `scripts/test/console-smoke-check.mjs` | green on the pushed commit — counts re-measured 2026-09-24 (they read panel 806 / gateway 917 before, and NOTHING gates them: the gate commands pass with a stale number in this cell, which is this file's own warning about counts, applied to itself). `console-smoke-check` is the newest NAMED gate: it discovers every `*render-smoke.mjs` in `gateway/ui` and runs all four, because CI ran exactly one of them and `overview-render-smoke.mjs` recorded the cost in its own comment — two of its checks, one of them the honesty rule the file exists for, were asserting on the empty string |
