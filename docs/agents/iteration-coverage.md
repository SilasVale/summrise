...
- Round log head: **round 265** (PRODUCT: a monitor can ask a WEB UI what it answers — an optional HTTP `path` turns the probe into a real GET and records the status code, so `401 up` and `500 down` are told apart where a TCP connect saw one thing. Verified on d1 against the agent's own server, a purpose-built 500, and the operator's ONU. Round 264 had been: attention that follows you OUT OF THE TAB — a live count in the tab title, a badge on the panel's icon, and a desktop notification for a watched host going down or an AI waiting on a human; permission is four states and a refusal is never drawn as "on". The device found a real bug in it (two notifications for one outage, because the push re-armed the poll's key) and the rule became one key per STATE. Round 263 had been: the monitor gets a VOICE — the device ANNOUNCES a watched target changing state over the SSE stream it already had (a banner in an open panel), keeps an outage log with wall-clock times and durations, and hands the same log to the AI through `monitor_list`; verified on d1 with the device speaking unprompted. Round 262 had been: reachability became CALLABLE — a `monitor` plugin exposes the watches over MCP (`monitor_list`/`probe`/`add`/`remove`), registered in the console MCP with its routing predicate and its three gateway gates answered; plus a `drops` rule and a `flapping` chip marked by a HOLLOW diamond so unstable is told apart from down by shape. Verified on d1 with a real probe→act→probe flapping series. Round 261 had been: the device WATCHES REACHABILITY — `monitor.rs` persists a bounded host:port list, probes each over TCP every 15 s into a bounded series, and the panel draws a Reachability card (latency line with GAPS for failed probes, state + duration) plus a strip chip that names a down target; verified on d1 against the operator's own ONU and a closed port, persistence proven by an update restart. Round 260 had been: the terminal gained CONSOLE KEYS — ^C/Enter/^D/^Z/Tab/Esc and, on a serial line, a real BREAK asserted through `TermBackend::send_break` and `terminal_write.break_ms`, advertised in the console MCP as well; verified on d1 with a foreground command actually interrupted and the break actually asserted on COM4. Round 259 had been: TWO DELIVERIES: the device can be UPDATED FROM ITS OWN PANEL — `GET /api/update` with the update plugin's own rules, and a button that POSTs the same `agent_update` tool an AI calls — and, out of the operator's own report, **the serial-port lock that made d1's COM4 refuse every open**: `SerialPool::open` mints a new entry id on every reconnect while the backend kept the id it was born with, so close released a stale entry and the live one kept its OS handle for the life of the process. Fixed with a `PoolEntry` guard that follows the session, and the failure message now separates BUSY from MISSING by presence. Round 258 had been: the device keeps a VITALS SERIES — `metrics.rs` owns the sampling clock (30 s, bounded ring), `GET /api/vitals/history` serves it oldest-first with its cadence, `lib/spark.ts` owns the gap-breaking geometry and the sustained-load rule, and the panel gained a **Device health** card plus a load chip on the strip; release 1.2.369 verified on d1 under a REAL 12-core load. Round 257 had been: the device keeps a RESTART HISTORY — `logs/boot-history.jsonl` as the fifth jsonl-family member, `GET /api/boots` with its one-place 24 h summary, a **Restarts** card in Settings and the strip's crash chip naming a repeated crash; release 1.2.368 on d1, card read out of the device's own browser. The same round repaired the coverage scope: an EXCLUDED string-prefix had been silently excluding the whole panel SPA, so the headline is now 349/363 with 87 never named — PRODUCT: the boot verdict reaches the wire, the panel and the fleet — `classify` answers first-run / clean-exit / replaced / machine-restart / crashed, `/api/status` carries `last_boot` + `last_boot_kind` unconditionally, the panel shows a crash chip and a five-minute "just restarted" one, and the console fleet marks the one exception it can act on; release 1.2.367 published and verified on d1, chip read out of the device's own browser). **This line said `round 121` until round 213 — a 91-round drift, and
...
- **A "STILL OPEN" LINE IS A CLAIM, NOT A FACT — three instances now, and round 245 named the CAUSE.**
  The tail of each journal entry is a COPY of the previous round's tail: copying is what makes it cheap
  and what stops it being re-measured. Round 245 swept every such line: round 239 still listed an index
  deploy that round 240 performed and verified by byte equality, and "convergence rows 6-7, which wait on
  a device event" travelled verbatim through rounds 240/241/242 while round 244 measured row 6 at ≤6 s.
  **Treat a carried-forward item as unverified until it is re-read.** The original note follows. Round 115 found memory
...
| agent `plugins/terminal` (26 tools) | partial | 88, SOLID, 259, 260 | **259 fixed a serial HANDLE LEAK**: the pool entry minted by an auto-reconnect was released by nobody (the backend held the id it was born with), so a closed session left COM4 locked for every program on the device until the agent restarted — measured on d1 from an operator report. The entry is now a `PoolEntry` guard that follows the session across reconnects, and an open failure says BUSY vs MISSING by device presence (the crate folds both OS errors into one kind and the text is localised). **260 added the BREAK path** (`send_break` on the backend trait, `break_ms` on `terminal_write`, the console-MCP schema and the panel's key bar) — the one console signal a browser cannot produce |
...
| agent `monitor.rs` (reachability) | seen | 261, 262 | targets persisted, 15 s TCP probes, bounded series, `GET /api/monitors` + add/remove/probe, **and since 262 a `monitor` PLUGIN puts all four on MCP** (registered in the console registry + the routing predicate, gateway redeployed) with a `drops` rule and the `flapping` chip. Still open: host:port, and since 265 optionally an **HTTP path with a status code** (401 up / 500 down are told apart; no CONTENT check yet, so a login page answering 200 still reads healthy). No latency threshold. **263 added the push**: a state change is announced on the event bus, the panel shows it as a banner, and the outage log carries durations. Still open: nothing reaches the operator when no window is open |
| agent `web/` (auth, panel grant, SSE, api) | seen | 88, 92, 256, 257, 258, 259 | grant single-use / audit record; **257 added `GET /api/boots`** (the device's restart history + a 24 h summary computed device-side); **259 added `GET /api/update`** (the update state a panel reads, with the plugin's own rules); **258 added `GET /api/vitals/history`** and moved `/api/status`'s readings onto the sampler's clock (one delta window for every consumer); `/api/status` carries the boot verdict (`last_boot` + `last_boot_kind`) UNCONDITIONALLY — 256 found the field written inside the `pending_approvals > 0` guard, i.e. unreachable in the only state a healthy device is ever in, and the endpoint test now asserts the pair on a response with zero pending approvals |
| agent `winmain.rs` boot path + `tunnel.rs` + `runstate.rs` | partial | 110, 112, 113, 254, 256 | tunnel F1/F2 closed (113). **The runtime half moved in 256: the boot task's own revival was OBSERVED on d1** — `vale stop` took the agent down, the watchdog brought it back ~2 min later, and the next boot classified it `crashed` from the real numbers (last heartbeat 125 s before the new start, survived 121 s), which `/api/status` then served and the panel rendered. 254 taught `describe_previous` to discriminate; 256 added `machine-restart` (host boot time beats the freshness heuristic) so a routine reboot cannot raise the crash chip. What is still pin-only: the 5-minute repetition trigger's own timing under a longer outage, and the supervisor half. **257 added the HISTORY (`logs/boot-history.jsonl`, the fifth jsonl-family member, bounded at 200 with a repairing prune) — and with it the row's one real gap: a DELIBERATE stop that outlives the classifier's minute still reads as `crashed` (measured: a `vale restart` on d1 whose revival took 95 s), because `mark_exited` has no production caller. The fix is a device route the CLI calls before stopping, dual-accepting an older agent.** |
...
| agent panel SPA (`resources/panel-react`) | seen | 90–93, 257 | **257 was the first round its `.ts` files could even be COUNTED**: an `EXCLUDED` string-prefix (`agent/resources/panel`, build output) had been silently matching `panel-react` since round 236, so the root was printed in the scope while ~63 of its files were excluded from the count. Repaired with a separator-aware `excludedDir()`. Its `.tsx` COMPONENTS are still out of `EXTS` — a stated scope decision, not a bug: adding `.tsx` would move the headline to 435/449 and the never-named queue to 155 |
...
### The user's ideas inbox — `docs/agents/ideas.md` (round 251)

**The objective makes this file the loop's TOP-PRIORITY input and forbids dropping an entry
silently. Round 251 measured how it has been treated: `grep -c 'ideas.md'` over `agent/AGENTS.md`
and THIS ledger returned ZERO and ZERO — the file was created as the fix for round 100's "aurora"
drift, and in the 151 rounds since, its records never named it once.** Current contents: **one entry,
status `standing`** ("The project should iterate and design itself; my creative direction enters
through here" → landed in `docs/CHARTER.md` + `docs/agents/iteration-loop.md`).

**The instrument is `agent/tests/ideas_inbox.rs`**, and it asserts the two mechanical halves of the
inbox's own rules: the loop's records must NAME the file (so a reader can tell "considered" from
"forgot"), and any entry whose status is not `standing` must be named in the round log (the rule that
was written down and never enforced). **It was RED on its first run and green once this entry and the
round-251 journal entry existed — the red state was the finding.**

**A new entry here needs no round of its own to be recorded — but it must be reflected here and in the
round log, and the test is what makes that non-optional.**
...
**MEASURED ROUND 236, RE-MEASURED ROUND 264: 361 of the repository's 375 non-excluded files are in scope (96%), and 93 are never named.** *(257 found that the scope had been lying about itself since 236 — an EXCLUDED string prefix was silently excluding the whole panel SPA; the repair moved the numbers 286/363 -> 349/363 and the queue 38 -> 87 in one round, because 63 files nobody could ever have named came into view at once.)* **The lie was a bare string prefix.** `EXCLUDED` names `agent/resources/panel` (BUILD OUTPUT — `panel.js` is embedded at compile time) and the walk tested it with `startsWith`, so `agent/resources/panel-react` — the panel SPA's SOURCE, listed in `ROOTS` since round 236 — matched it and **all ~63 of its `.ts` files were silently excluded from a scope whose own output printed that root on every run**. The denominator already appended `"/"`; the walk did not, so the two halves of one rule disagreed and only the permissive one was ever visible. Round 236 had already fixed this shape one function over (EXCLUDED applying only inside `catch`), which is why the repair is the shared `excludedDir()` predicate rather than a second `+"/"`: fixing the number is the small half. The scope went 286/363 -> 349/363 and the never-named queue 38 -> 87, because 63 files nobody had ever looked at came into view at once. **Two boundaries are stated here rather than implied: `.tsx` is still out of `EXTS` (so the two React SPAs' components are counted OUT while their hooks and libs are counted IN — adding it would move the headline to 435/449 and the queue to 155, a scope decision rather than a bug fix), and generated/vendored/retired/build-output trees stay excluded WITH REASONS, printed every run.** **The scope moves every time a round adds a file under an in-scope root, and the NUMERATOR has now done so FOUR times: 282 -> 283 in round 245 (`agent/tests/coverage_numbers.rs`), -> 284 in round 246 (`agent/tests/guide_build_commands.rs`), -> 285 in round 251 (`agent/tests/ideas_inbox.rs`), and the panel SPA's 63 in round 257. **The DENOMINATOR counts the COMMIT (`git ls-files`, round 256), not the working directory, which is why those older rounds' denominators read four higher than the 363 measured today.** Every one was caught by the instrument round 245 added, before any reader saw it. That is this paragraph's own point demonstrated on itself, and the last three times it was CAUGHT BY THE INSTRUMENT ROUND 245 ADDED — which is the difference between stating a limit and enforcing one. The first two numbers are STABLE (the scope changes only when `ROOTS` changes) and `agent/tests/coverage_numbers.rs` asserts them against the tool. **The third is NOT asserted and must be re-measured before this sentence is trusted: it was 52 when round 236 wrote it, 39 by round 245, 38 by round 256 and 91 now — most of that jump is round 257's scope repair, not a regression: 63 files nobody had ever been able to name came into view at once, because it falls only when some round's entry happens to NAME a file** — the tool's own stated limit, "it measures 'named', not 'examined'". A number that legitimately moves has no stable subject to assert (ADR 0011's deletion criterion) — tests joined the scope (ADR 0012), because they were the bulk of what round 235 found missing and `agent/tests/` holds this loop's own instruments. Generated, vendored, retired and build-output trees are excluded WITH REASONS, printed every run. (Round 235's measurement, kept for the record: **154 of the repository's 415 source files are in scope (37%), and 8 are never named.** The 261 outside the scope are `gateway` 122, `agent` 116, `index` 15, repo root 4, `proxies` 2, `docs` 2 — tests, deployment scripts and the panel SPA, a defensible boundary that **nothing had ever stated**, which is how "every source file in scope is named" kept being read as a statement about the repository. The run prints the ratio and the omitted directories, and removing a root from `ROOTS` now makes it appear there. (Round 234's measurement: **154 files in scope, and 10 never named** — round 234 added `scripts/`, which had never been in the scope at all, so every earlier verdict here was silent about the 19 files holding every check CI runs. `.bash` and `.py` joined the extension list for the same reason. (Round 230's measurement, kept for the record: **135 files in scope, and 12 never named.** Round 229 reported 118 and zero — and its scope listed `proxies/api-relay/src`, **a directory that does not exist**, which `walk()` skipped silently, so the whole `api-relay` subsystem (17 files, kept in `api/` and `server/`) went uncounted. **The zero was a verdict about a scope that was not the one stated, and the tool now exits 2 when a root cannot be read, because typo'ing a root used to take the file count from 118 to 77 while still reporting `never named: 0` — a plausible, lower number with a clean verdict.** All 12 sit under `api-relay`, the subsystem CI only `node --check`s. (Round 229's original sentence follows, superseded: **118 files in scope, and ZERO never named** — the table's own population-level
...
0011 two artifacts, one obligation (round 224) · 0012 tests are surfaces (round 236) · interactive-browser
