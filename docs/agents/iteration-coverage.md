...
| agent `monitor.rs` (reachability) | seen | 261 | new this round: targets persisted, 15 s TCP probes, bounded series, `GET /api/monitors` + add/remove/probe. **Panel-only — no MCP tool exposes it**, so an AI driving this device cannot add or read a watch (the console-MCP visibility decision, deliberately untaken) |
