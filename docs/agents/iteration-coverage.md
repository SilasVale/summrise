...
| agent `monitor.rs` (reachability) | seen | 261, 262 | targets persisted, 15 s TCP probes, bounded series, `GET /api/monitors` + add/remove/probe, **and since 262 a `monitor` PLUGIN puts all four on MCP** (registered in the console registry + the routing predicate, gateway redeployed) with a `drops` rule and the `flapping` chip. Still open: host:port only — no HTTP status check and no latency threshold. **263 added the push**: a state change is announced on the event bus, the panel shows it as a banner, and the outage log carries durations. Still open: nothing reaches the operator when no window is open |
