...
| agent `web/` (auth, panel grant, SSE, api) | seen | 88, 92, 256, 257, 258 | grant single-use / audit record; **257 added `GET /api/boots`** (the device's restart history + a 24 h summary computed device-side); **258 added `GET /api/vitals/history`** and moved `/api/status`'s readings onto the sampler's clock (one delta window for every consumer); `/api/status` carries the boot verdict (`last_boot` + `last_boot_kind`) UNCONDITIONALLY — 256 found the field written inside the `pending_approvals > 0` guard, i.e. unreachable in the only state a healthy device is ever in, and the endpoint test now asserts the pair on a response with zero pending approvals |
