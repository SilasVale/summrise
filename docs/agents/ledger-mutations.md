<!-- LEDGER ARCHIVE 3 of 4 — the mutation table.
     ...
     The other three archives are docs/agents/design-ledger.md (the recent rounds and the exploration series),
     docs/agents/ledger-appendix.md (the lookup tables) and docs/agents/ledger-early-rounds.md (rounds 54-162, moved
     out in round 273). Titles still resolve across all four; ledger-budget-check.mjs carries a FLOOR and a CEILING
     for each — and the ceiling half of that sentence was FALSE until round 273, when the check that applies it was
     found sitting below the only consumer of its result. See the two `ledger-budget-check` rows below. -->
...
| `scripts/test/workflow-yaml-check.mjs` (new, round 273) | write a gate into `ci.yml` as an orphaned command line under a previous step's `run:` — the real defect, replanted character for character from `e92fbf32` | exit 1, naming the file and the line: `GITHUB CANNOT PARSE THIS FILE, SO NO JOB IN IT WILL EVER START`. **THE CONSEQUENCE THIS PROVES AGAINST IS NOT A RED STEP, IT IS NO STEPS**: the real occurrence left `ci.yml` unparseable for **~35 commits**, and GitHub answered every push with a completed run carrying **ZERO jobs** and `conclusion: failure` — no log to open, and indistinguishable in a count from a real red (`8ca63398` ran 11 jobs and succeeded; `e92fbf32` ran 0). TWO GATES COULD NOT SEE IT, and both are named here so the next reader does not re-derive them: `workflow-shell-check.mjs` extracts `run:` blocks and a line with no `run:` key is not one — while its header CLAIMED "the extraction goes through the YAML PARSER" and its own body said "Using a real parser would be better"; and `all-gates.bash` derives its list from the same file by regex, so it ran all 61 commands happily on a file GitHub had rejected. **The instrument read the file as TEXT; the other end reads it as YAML.** The gate now uses a real parser where the host has one and a portable fallback where it does not, and the fallback is DIFFERENTIALLY PROVEN rather than asserted: `--differential` replays every workflow revision in this repository's history through both — **293 revisions across 148 commits, zero disagreements**, the 35 broken ones included, so the reject direction is exercised. On the real broken bytes both instruments name the SAME line (663) |
| `scripts/test/ledger-budget-check.mjs` (the FIRST archive's ceiling — the one that was DEAD) | move the `ARCHIVE_CEILING` check back below the `if (failures.length) { … process.exit(1) }` block, where it sat from the day it was written until round 273 | exit 1 → **exit 0, while printing `ok — … (457929 B of 400000)`**. That is the whole defect in one line: the check RAN, computed the RIGHT number, pushed it onto an array nothing read again, and the gate reported success on an archive **57,929 bytes over its own limit**. **NO MUTATION HAD TO BE PLANTED FOR THIS ONE**, and that is the strongest form the proof takes: the tree at round 273's start was already over the ceiling, so re-arming the check turned a GREEN gate RED on an UNCHANGED tree — the before/after is the same file, byte for byte. Rounds 188 and 189 both read this gate's output and recorded the opposite conclusion ("this file has no byte ceiling"), which is what a check placed after the only consumer of its result produces: **a message that always agrees.** The boundary-crossing mutation now lives in `gate-mutations-check.mjs` (90,001 bytes padded onto the ledger → exit 1), so a re-arm that is undone fails CI rather than waiting for another round to read the line |
...
| `scripts/test/skill-frontmatter-check.mjs` — the TRIGGER arm (rounds 174) | give an INVOCABLE skill (one with no `disable-model-invocation`) a description that merely describes it — e.g. replace `tdd`'s with "A calm, careful approach to building things well." | exit 1: "tdd: invocable by the model but its description names no TRIGGER — it describes the skill instead of saying when to reach for it". **AND THE NON-BITE HOLDS**: the SAME description plus `disable-model-invocation: true` PASSES — measured, because a gate that cannot tell those two apart gets reverted |
| `scripts/test/agents-snippet-check.mjs` (new, rounds 186-187) | break any LABELLED code block in `AGENTS.md` or `agent/AGENTS.md` — e.g. drop a `}` from the `npm test >/tmp/out 2>&1 || { echo FAILED; exit 1; }` form | exit 1: "1 labelled block(s) that do not parse - a reader would run this and fail". **AND THE NON-BITE HOLDS**: the UNLABELLED Agent-layout DIRECTORY TREE passes — only a labelled block claims to be code, and a check that flagged the diagram would be reverted |
