...
describe("RunStrip — one row per run", () => {
  it("renders the label, the goal, the state and the per-feed counts", async () => {
    ...
    expect(row.querySelector(".run-row-label")!.textContent).toBe(
      "provision the ONU",
    );
    expect(row.querySelector(".run-row-goal")!.textContent).toContain(
      "get it online",
    );
    expect(row.querySelector(".run-row-state")!.textContent).toBe("closed");
    expect(
      [...row.querySelectorAll(".run-row-count")].map((c) => c.textContent),
    ).toEqual(["2 terminal", "1 browser"]);
    // The span is the run's OWN extent, not a live one.
    expect(
      row.querySelector(".run-row-span")!.getAttribute("data-start-ms"),
    ).toBe(String(T0));
    expect(
      row.querySelector(".run-row-span")!.getAttribute("data-end-ms"),
    ).toBe(String(T0 + 60_000));
  });
  ...
  it("shows the outcome a closed run recorded", async () => {
    ...
    const { container } = await mount(<RunStrip pollMs={60_000} />);
    await waitFor(() =>
      expect(container.querySelector(".run-row-outcome")).not.toBeNull(),
    );
  ...
  it("draws NOTHING where an absent outcome would go", async () => {
    ...
    device(
      [ev({ ts_ms: T0 + 1, run_id: "r-a" })],
      [begin("r-a", T0), end("r-a", T0 + 9_000)],
    );
    const { container } = await mount(<RunStrip pollMs={60_000} />);
    await waitFor(() =>
      expect(container.querySelectorAll(".run-row")).toHaveLength(1),
  ...
  it("renders an OPEN run as open, with the honesty line and a non-ticking span", async () => {
    ...
    expect(row.querySelector(".run-row-note")!.textContent).toBe(
      "no end recorded",
    );
  ...
  it("renders an UNREGISTERED run under its raw id", async () => {
    ...
    expect(row.querySelector(".run-row-state")!.textContent).toBe(
      "unregistered",
    );
  ...
  it("marks an unlabeled run EXPLICITLY instead of leaving its name blank", async () => {
    ...
    const row = container.querySelector(".run-row")!;
    expect(row.querySelector(".run-row-unlabeled")!.textContent).toBe(
      "unlabeled",
    );
...
describe("RunStrip — the unattributed bucket", () => {
  it("is a row of its OWN, after the runs, never folded into the one above it", async () => {
    ...
    expect(
      [...rows[0].querySelectorAll(".run-row-count")].map((c) => c.textContent),
    ).toEqual(["2 terminal", "0 browser"]);
    expect(rows[1].getAttribute("data-state")).toBe("unattributed");
    expect(rows[1].querySelector(".run-row-state")!.textContent).toBe(
      "unattributed",
    );
    expect(rows[1].querySelector(".run-row-unlabeled")!.textContent).toBe(
      "no run id",
    );
    expect(
      [...rows[1].querySelectorAll(".run-row-count")].map((c) => c.textContent),
    ).toEqual(["1 terminal", "1 browser"]);
  ...
  it("renders alone when the device recorded no runs at all", async () => {
    ...
    expect(
      container.querySelector(".run-row")!.getAttribute("data-state"),
    ).toBe("unattributed");
  });
});
...
describe("RunStrip — section behaviour", () => {
  ...
  it("summarises open runs without expanding anything", async () => {
    ...
    const { container } = await mount(<RunStrip pollMs={60_000} />);
    await waitFor(() =>
      expect(container.querySelector(".run-strip-open")).not.toBeNull(),
    );
    expect(container.querySelector(".run-strip-open")!.textContent).toBe(
      "2 open",
    );
...
describe("PathView integration", () => {
  ...
  it("puts the run strip at the very top of the path view", async () => {
    device(
      ...
      [begin("r-a", T0, { label: "the run" })],
    );
    const { container } = await mount(<PathView events={cmdEvents} />);
    await waitFor(() =>
      expect(container.querySelector(".run-strip")).not.toBeNull(),
    );
  ...
  it("still shows the device's runs when this session has no path yet", async () => {
    ...
    device(
      [],
      [
        begin("r-a", T0, { label: "another session's run" }),
        end("r-a", T0 + 10),
      ],
    );
