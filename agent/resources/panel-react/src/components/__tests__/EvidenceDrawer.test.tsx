...
describe("EvidenceDrawer", () => {
  ...
  it("fetches shots + actions when opened and renders the timeline", async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url.includes("/api/browser/pwshots")) {
        return new Response(
          JSON.stringify({ shots: [{ name: "page-1.png", mtime_ms: 1000 }] }),
          { status: 200 },
        );
      }
  ...
  it("refreshes on the summrise-* window events, not its own SSE stream (P1-3)", async () => {
    ...
    await waitFor(
      () => expect(screen.getByText(/AI screenshots \(0\)/)).toBeTruthy(),
      { timeout: 3000 },
    );
