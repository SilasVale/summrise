...
async fn list_tools_via_http() {
    ...
    assert_eq!(tools.tools.len(), 57, "27 terminal (incl. terminal_env + terminal_jobs + terminal_plan + terminal_sftp + terminal_forget_saved + terminal_secret_* aliases) + agent_update + page_view + 4 mcp_client_* + 2 playwright + 6 memory_* + 9 system_* (round-266 added system_file_stat, round-340 added system_file_download, round-341 added system_file_upload) + 2 runs_* (run_begin + run_end — the AI-execution identity pair) + 5 monitor_* (round-262: monitor_list + monitor_probe + monitor_add + monitor_remove — the reachability watches, which the panel could see since round 261 while the AI's only channel, MCP, could not; round-19x added monitor_note, the operator's own words next to the device's observation)");
    ...
    // tool that exists in the registry but not on the wire would make the whole
    // feature dead on arrival.
    for name in ["run_begin", "run_end"] {
        assert!(
            tools.tools.iter().any(|t| t.name == name),
            "{name} must be listed over MCP — the AI's only channel to this device"
        );
    }
