...
pub struct ServerConfig {
    ...
    /// sends this as X-Summrise-Auth when proxying /panel/ so the agent can
...
impl MemoryConfig {
    ...
    /// store.
    ///
    /// The literals twin summrise-agent's MemoryLimits::default — pinned by the
...
mod tests {
    ...
    fn load_rejects_port_zero() {
        let dir = std::env::temp_dir().join(format!("summrise-cfg-test-{}", std::process::id()));
    ...
    fn platform_decouple_and_server_defaults() {
        ...
        // because CI ran `cargo test -p summrise-agent` and never this crate (now fixed too).
