    ...
    pub fn new(size: COORD, input: FileDescriptor, output: FileDescriptor) -> Result<Self, Error> {
        ...
        let result = unsafe {
            (CONPTY.CreatePseudoConsole)(
                ...
                // (BEL/ESC \) as a keystroke and renders the `>>` continuation
                // prompt after every prompt (d1, VS Code shell integration).
                // node-pty (VS Code) passes flags=0/INHERIT_CURSOR only —
                // mirror that: no WIN32_INPUT_MODE.
