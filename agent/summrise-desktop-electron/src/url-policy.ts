...
// chain copies it, and `summrise update` stages + swaps it with main/preload.
//
...
// SUMMRISE_AGENT_PORT env, else the registry install/data dirs). Predicates
...
export function controlOriginOk(origin: string | undefined | null): boolean {
  if (!origin || origin === "null") return true; // native tooling / the data: wait page
  try {
