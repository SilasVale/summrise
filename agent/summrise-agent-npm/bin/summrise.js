...
exports.asciiJson = asciiJson;
exports.deviceApi = deviceApi;
exports.fmtDuration = fmtDuration;
exports.targetLine = targetLine;
exports.monitorsJson = monitorsJson;
exports.probeLine = probeLine;
...
function resolveDir() {
    if (process.env.SUMMRISE_AGENT_DIR)
        return process.env.SUMMRISE_AGENT_DIR;
    try {
        const out = (0, child_process_1.spawnSync)("reg", ["query", "HKLM\\SOFTWARE\\Summrise\\Agent", "/v", "InstallDir"], { encoding: "utf8" });
...
function resolveDataDir() {
    try {
        const out = (0, child_process_1.spawnSync)("reg", ["query", "HKLM\\SOFTWARE\\Summrise\\Agent", "/v", "DataDir"], { encoding: "utf8" });
        if (out.status === 0 && out.stdout) {
...
function regWrite(name, value) {
    ...
    if (r.status !== 0) {
        console.error(`setup: WARNING -- could not record ${name} in HKLM\\SOFTWARE\\Summrise\\Agent` +
...
function asciiJson(value, indent) {
    return JSON.stringify(value, null, indent).replace(/[\u007f-\uffff]/g, (c) => "\\u" + c.charCodeAt(0).toString(16).padStart(4, "0"));
}
...
function monitorsJson({ device, askedAtMs, payload, only }) {
...
function probeLine(target, probe, nowMs) {
...
function bootTaskPs(exeQ, cfgQ, start = false) {
    ...
    if (start)
        lines.push("Start-ScheduledTask SummriseAgent");
    return lines;
}
...
function migrateLayoutPs(q, dq) {
    const mvf = (oldRel, newAbs) => `if ($summriseMg -and (Test-Path '${q}\\${oldRel}') -and (-not (Test-Path '${newAbs}'))) { try { New-Item -ItemType Directory -Force -Path (Split-Path '${newAbs}') | Out-Null; Move-Item -Force -Path '${q}\\${oldRel}' -Destination '${newAbs}' -ErrorAction Stop } catch {} }`;
    const mvd = (oldRel, newAbs) => `if ($summriseMg -and (Test-Path '${q}\\${oldRel}')) { try { if (-not (Test-Path '${newAbs}')) { New-Item -ItemType Directory -Force -Path (Split-Path '${newAbs}') | Out-Null; Move-Item -Path '${q}\\${oldRel}' -Destination '${newAbs}' -ErrorAction Stop } else { Get-ChildItem -Force '${q}\\${oldRel}' | ForEach-Object { if (-not (Test-Path (Join-Path '${newAbs}' $_.Name))) { Move-Item -Force -Path $_.FullName -Destination (Join-Path '${newAbs}' $_.Name) -ErrorAction SilentlyContinue } } } } catch {} }`;
    const etc = `${q}\\etc`;
...
exports.BOOT_TASKS = ["SummriseAgent", "SummriseDesktop"];
...
function ensureElectron() {
    ...
    if (fs.existsSync(path.join(dist, "electron.exe")))
        return dist;
    const zip = resolveComponent("electron-win32-x64.zip", path.join(__dirname, "..", "electron-win32-x64.zip"));
    if (!zip)
        return null;
    ...
    ps(`Expand-Archive -Force -Path '${(0, exports.psq)(zip)}' -DestinationPath '${(0, exports.psq)(dist)}'`);
    if (!fs.existsSync(path.join(dist, "electron.exe")))
        return null;
    try {
...
function statusReport(f) {
    ...
    if (f.latestVersion == null) {
        ...
        out.push(`latest: ${f.latestVersion} is on the CDN -- THIS DEVICE IS BEHIND by ${behindBy(f.releaseVersion, f.latestVersion)}; run 'summrise update'`);
    }
...
function stageDesktopShell(installDir, suffix) {
    ...
    if (!fs.existsSync(DESK_SRC))
        return 0;
    const desDst = path.join(installDir, "components", "summrise-desktop-electron", "src");
    ...
    for (const icon of ["icon.png", "icon.ico"]) {
        ...
        if (fs.existsSync(iconSrc))
            fs.copyFileSync(iconSrc, path.join(installDir, "components", "summrise-desktop-electron", icon));
    }
...
const commands = {
    ...
    setup(args) {
        ...
        const BUSY = path.join(process.env.ProgramData || "C:\\ProgramData", "SummriseAgent", "update-busy");
        ...
        ps(`Remove-Item -Force -ErrorAction SilentlyContinue '${(0, exports.psq)(BUSY)}'`);
        ...
        ps(`Get-CimInstance Win32_Process | Where-Object { $_.Name -eq 'node.exe' -and $_.CommandLine -like '*${(0, exports.psq)(DIR)}*playwright*' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }`);
        ps(`Remove-Item -Recurse -Force -ErrorAction SilentlyContinue '${(0, exports.psq)(PW_DIR)}'`);
        ...
        // instance between attempts.
        {
            ...
            for (let i = 0; i < 12; i++) {
                try {
                    ...
                    (0, child_process_1.spawnSync)("cmd", ["/c", "taskkill", "/F", "/IM", "summrise-agent.exe"], {
                        stdio: "ignore",
                    });
                    try {
            ...
            if (!copied) {
                console.error("setup: FATAL -- could not replace summrise-agent.exe (locked). Close any running Summrise agent and re-run the installer.");
                process.exit(1);
            }
        }
        ...
        const PW_ZIP = resolveComponent("summrise-playwright.zip", path.join(__dirname, "..", "summrise-playwright.zip"));
        if (PW_ZIP) {
            const pwDir = PW_DIR;
            fs.mkdirSync(pwDir, { recursive: true });
            // argv (`ps()`): PW_ZIP and COMPONENTS_DIR are paths, and cmd expands `%NAME%`
            // inside quotes (see `sh()`).
            ps(`Expand-Archive -Force -Path '${(0, exports.psq)(PW_ZIP)}' -DestinationPath '${(0, exports.psq)(COMPONENTS_DIR)}'`);
            ...
            console.error("setup: WARNING -- the playwright bundle could not be obtained (not in the package, and the release host did not serve it); browser tools stay disabled.");
        }
        ...
        const CF_SRC = resolveComponent("cloudflared.exe", path.join(__dirname, "..", "cloudflared.exe"));
        if (CF_SRC) {
            fs.mkdirSync(COMPONENTS_DIR, { recursive: true });
            fs.copyFileSync(CF_SRC, path.join(COMPONENTS_DIR, "cloudflared.exe"));
            console.log("setup: cloudflared staged (tunnel optional -- `summrise tunnel install` to enable)");
        }
        else {
            ...
            console.error("setup: WARNING -- cloudflared could NOT be staged (not in the package, and the release host did not serve it); this device will be unreachable from the console until the binary can be fetched.");
        }
        ...
        try {
            ...
            const reg302 = (0, child_process_1.spawnSync)("powershell", ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", reg], { stdio: "pipe" });
        ...
        try {
            ...
            console.log("setup: control-panel entry skipped (uninstall via `summrise uninstall`)");
        }
    ...
    desktop() {
        ...
        const r = (0, child_process_1.spawnSync)("powershell", ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", start], { encoding: "utf8" });
    ...
    autostart(args) {
        ...
        if (skipped === exports.BOOT_TASKS.length) {
            console.error("autostart: no boot tasks found -- nothing to switch. `summrise setup` registers the agent task; the desktop task comes from the installer.");
            process.exit(1);
        }
        // Only claim the durable outcome when every EXISTING task actually changed.
        if (sub === "off" && !failed)
            console.log("autostart: off -- tasks stay disabled across reboot until 'summrise autostart on'");
        if (failed)
            process.exit(1);
    },
    async update() {
        ...
        if (!receipt || receipt.status !== 0 || receipt.error) {
            console.error("update: WARNING -- could not write the receipt to summrise-update.log (" +
        ...
        const script = [
            ...
            ...deskShortcutRepairPs(`${q}\\scripts`, `${q}\\components\\summrise-desktop-electron`, log),
    ...
    async rollback(args) {
        ...
        console.log(`rollback: installing summrise-agent ${val} into ${NPM_GLOBAL} ...`);
        ...
        if (!fs.existsSync(summriseCmd)) {
            console.error("rollback: summrise.cmd missing after install (broken package?) -- aborting before any swap");
            process.exit(1);
        }
        console.log(`rollback: swapping in ${val} (connection drops ~10s) ...`);
        const upd = (0, child_process_1.spawnSync)(summriseCmd, ["update"], {
            stdio: "inherit",
            timeout: 120000,
        });
        if (upd.status !== 0) {
        ...
        if (pinned) {
            ...
            if (back !== val) {
                console.error(`rollback: WARNING -- wrote ${PIN} but read back ${back ?? "nothing"}; agent_update may not be blocked`);
            }
            else {
            ...
            try {
                ...
                console.error(`rollback: note -- the pin is in place, but the pre-v2 root marker could not be removed (${e.message}); it is inert`);
            }
        }
    },
    ...
    uninstall(args) {
        ...
        if (!fs.existsSync(path.join(DIR, "summrise-agent.exe")) &&
        ...
        sh(`powershell -NoProfile -Command "Get-CimInstance Win32_Process | Where-Object { $_.Name -eq 'node.exe' -and ($_.CommandLine -like '*summrise-agent*playwright*' -or $_.CommandLine -like '*summrise-command*playwright*') } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }"`);
        ...
        ps(`Remove-Item -LiteralPath '${(0, exports.psq)(DIR)}' -Recurse -Force -ErrorAction SilentlyContinue`);
        ...
        const regLeft = (0, child_process_1.spawnSync)("reg", ["query", "HKLM\\SOFTWARE\\Summrise\\Agent"], {
            encoding: "utf8",
        });
        if (regLeft.status === 0)
        ...
        if (purge) {
            ...
            ps(`Remove-Item -LiteralPath '${(0, exports.psq)(DATA)}' -Recurse -Force -ErrorAction SilentlyContinue`);
    ...
    run(args) {
        ...
        if (!fs.existsSync(EXE_DST)) {
            console.error(`summrise run: agent binary not found at ${EXE_DST} -- run 'summrise setup' first`);
            process.exit(1);
        }
        ...
        if (r.error || r.status === null) {
            console.error(`summrise run: could not start the agent (${r.error ? r.error.message : "no exit status"})`);
            process.exit(1);
        }
    ...
    async tunnel(args) {
        ...
        switch (sub) {
            ...
            case "update": {
                console.log("tunnel: version is locked by the Summrise release flow -- update via the installer/npm package.");
                return;
            }
            default:
