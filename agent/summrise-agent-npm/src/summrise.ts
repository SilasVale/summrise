...
const commands = {
  ...
  setup(args) {
    ...
    regOk.push(
      regWrite(
        "DataDir",
        path.join(process.env.ProgramData || "C:\\ProgramData", "Summrise"),
      ),
    );
    ...
    try {
      ...
      sh(
        `powershell -NoProfile -ExecutionPolicy Bypass -File '${psq(reg)}'`,
      );
      console.log(
  ...
  desktop() {
    ...
    try {
      sh(`powershell -NoProfile -ExecutionPolicy Bypass -File '${psq(reg)}'`);
    } catch {
  ...
  stop() {
    ...
    if (r && r.status !== 0) {
      console.error(
        `summrise stop: schtasks /End failed (status ${r.status}) -- the agent may still be running`,
      );
      process.exit(1);
    }
    console.log(
