# ValeIntegrity.tests.ps1 — plain asserts, no Pester. Exit 0 = all green.
# Run: pwsh -File agent/deploy/lib/ValeIntegrity.tests.ps1
#
...
. "$PSScriptRoot/ValeIntegrity.ps1"
...
# --- the file verdict -------------------------------------------------------
$tmp = Join-Path ([IO.Path]::GetTempPath()) ("vale-integrity-" + [Guid]::NewGuid().ToString('N') + ".bin")
...
Write-Host "ValeIntegrity: $PASS checks passed"
