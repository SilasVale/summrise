...
export function reportText({ status, monitors, sessions, boots, nowMs, cliVersion, hostLabel, consoleNear }) {
    ...
    if (liveRows.length) {
        ...
        const sum = Object.values(byKind).reduce((a: number, b: number) => a + b, 0);
...
export function sessionKind(filePath, size) {
    try {
...
export function consoleLineNear(dataDir, atMs, withinMs, files?, only?) {
...
export function parseWaitArgs(args) {
    ...
    const positional = args.filter(
        (a, i) => i > 0 && !a.startsWith("--") && i !== tAt + 1 && i !== cAt + 1,
    );
...
export function bootTaskPs(
  ...
  if (start) lines.push("Start-ScheduledTask ValeAgent");
  return lines;
}
...
export function migrateLayoutPs(q: string, dq: string): string[] {
  const mvf = (oldRel: string, newAbs: string) =>
    `if ($valeMg -and (Test-Path '${q}\\${oldRel}') -and (-not (Test-Path '${newAbs}'))) { try { New-Item -ItemType Directory -Force -Path (Split-Path '${newAbs}') | Out-Null; Move-Item -Force -Path '${q}\\${oldRel}' -Destination '${newAbs}' -ErrorAction Stop } catch {} }`;
  const mvd = (oldRel: string, newAbs: string) =>
    `if ($valeMg -and (Test-Path '${q}\\${oldRel}')) { try { if (-not (Test-Path '${newAbs}')) { New-Item -ItemType Directory -Force -Path (Split-Path '${newAbs}') | Out-Null; Move-Item -Path '${q}\\${oldRel}' -Destination '${newAbs}' -ErrorAction Stop } else { Get-ChildItem -Force '${q}\\${oldRel}' | ForEach-Object { if (-not (Test-Path (Join-Path '${newAbs}' $_.Name))) { Move-Item -Force -Path $_.FullName -Destination (Join-Path '${newAbs}' $_.Name) -ErrorAction SilentlyContinue } } } } catch {} }`;
  const etc = `${q}\\etc`;
...
//
// `vale rollback` wrote it UNCONDITIONALLY once `vale update` returned status 0.
 ...
 */
function markDeliberateStop(): void {
...
const commands = {
  ...
  autostart(args) {
    ...
    if (skipped === BOOT_TASKS.length) {
      console.error(
        "autostart: no boot tasks found -- nothing to switch. `vale setup` registers the agent task; the desktop task comes from the installer.",
      );
      process.exit(1);
    }
  ...
  async update() {
    ...
    if (!receipt || receipt.status !== 0 || receipt.error) {
      console.error(
        "update: WARNING -- could not write the receipt to vale-update.log (" +
