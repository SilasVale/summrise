...
exports.chooseConsoleSession = chooseConsoleSession;
exports.advanceConsoleCursor = advanceConsoleCursor;
exports.waitDecision = waitDecision;
exports.waitLine = waitLine;
exports.parseWaitArgs = parseWaitArgs;
exports.monitorsJson = monitorsJson;
exports.stripAnsi = stripAnsi;
exports.lastLine = lastLine;
exports.lastOutputBefore = lastOutputBefore;
...
function reportText({ status, monitors, sessions, boots, nowMs, cliVersion, hostLabel, consoleNear }) {
...
function sessionTailRecords(file, capBytes = 128 * 1024) {
...
function chooseConsoleSession(sessions, explicit) {
...
function advanceConsoleCursor(cursor, res) {
...
function waitDecision({ ok, wantUp, elapsedMs, timeoutMs }) {
...
function waitLine({ elapsedMs, probe, wantUp, id, json }) {
 ...
 *  finished saying. */
function lastLine(text, cap = 120) {
...
function lastOutputBefore(records, atMs) {
    let best = null;
    ...
    async monitor(args) {
        ...
        if (sub === "wait") {
            ...
            for (;;) {
                ...
                if (decision === "met") {
                    if (!w.json)
                        console.log(`${w.target.id} is ${w.wantUp ? "up" : "down"} after ${Math.round(elapsedMs / 1000)}s`);
                    process.exit(0);
                }
                if (decision === "timed-out") {
                    console.error(`monitor wait: ${w.target.id} was still not ${w.wantUp ? "up" : "down"} after ${Math.round(elapsedMs / 1000)}s`);
                    process.exit(1);
                }
...
if (require.main === module) {
    ...
    if (!cmd || !commands[cmd]) {
        console.log("vale <setup|status|report|watch|monitor|start|stop|restart|autostart|update|rollback|uninstall|run|tunnel> -- Vale Agent control");
