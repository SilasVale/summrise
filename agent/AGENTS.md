...
file they had open**). Commit: helper.js + tools.rs + journal. agent 673, zero red, four steps green.
  (1) MY FIRST HYPOTHESIS WAS WRONG, AND SAYING SO IS PART OF THE RESULT: a file whose header says
  "GENERATED by vale-agent" while sitting in `src/` looked like a checked-in copy of an embedded template —
  two artifacts that could drift. **Measured: `tools.rs:54` is `const BROWSER_HELPER_JS: &str =
  include_str!("helper.js")`, so this file IS the source, there is exactly ONE copy in the repo, and
  `ensure_browser_helper` writes it out as `vale-browser-helper.js` — repairing the copy whenever its
  content differs.** No drift is possible. The hypothesis was the wrong question, as in round 217.
  (2) THE FINDING IS THAT ONE TEXT SERVES TWO ARTIFACTS AND DESCRIBES ONLY ONE OF THEM, WITHOUT NAMING IT.
  The header is part of `BROWSER_HELPER_JS`, so it ships to the generated copy — where "GENERATED by
  vale-agent; do not edit, it is overwritten" is TRUE. But the same text sits at the top of the SOURCE a
  developer opens in the repo, where all three claims are FALSE: nothing generates this file, it is not
  overwritten, and editing it is the ONLY way a change ships. **"This file" is the whole bug: the text never
  named which file it meant, so it was wrong for whichever one the reader had open — and the reader in the
  repository is the one who acts on it.**
  (3) THE CONSEQUENCE IS CONCRETE RATHER THAN COSMETIC: an engineer fixing a browser-helper bug reads "do
  not edit, it is overwritten" and either hunts for a generator that does not exist or edits the runtime
  copy in an output directory — which the next run repairs, silently discarding the fix. That is a
  wrong-comment bug whose cost is a lost change, not a wrong byte.
  (4) THE FIX NAMES THE COPY AND PINS THE DISTINCTION: the header now says this is the canonical source,
  gives the embedded path and the written filename, and records why the old wording was wrong; two
  assertions in the module's own test (round 183's rule — the harness was already there) require the header
  to CONTAIN `BROWSER_HELPER_NAME` and to NOT contain "do not edit, it is overwritten". **The first is the
  sharper one: the old text contained no filename at all, so that assertion would have failed on it.** The
  mutation is the historical text itself, and it turns the suite red with the message naming the source file.
  (5) SO THE ROUND-165 SWEEP CLOSES: all 15 rows of "121 files, 15 never named by any round" have now been
  opened — `body-scan.ts` and `approval.rs` (a SECURITY fix) in rounds 168/170, `model-route.ts`/
  `models-probe.ts` in 191/195, `store/file-config.ts` in 174-178, and in this stretch `http.ts` (rank 1),
  `lib/ratelimit.ts` (rank 4), `plugins/registry.ts` (rank 5), the three `vale-command-core` contract files
  (rank 9, opened in 214's course) and now `playwright/helper.js` (rank 8). **Three of them held up on
  inspection and are now pinned by assertions rather than by trust; one — this one — was genuinely wrong
  in its documentation.** STILL OPEN: the installer's signing decision (the user's call); the recurring
  second failure's name under mutation; `models-probe.ts`'s remaining body (the one round-165-adjacent item
  that is a CODE read rather than a surface, still unopened).
