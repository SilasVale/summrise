// panel-stub.cjs — the stubbed device API the harness page runs, as a real module (round 270).
//
// It was a 640-line template literal inside panel-render-audit.mjs with three interpolations (the session id, the
// session object and the event list), and two guards that existed only because of that: a search for a backtick
// ("a backtick in this template ends the literal and the file stops parsing" — round 154 wrote two of them into a
// comment ABOUT the other trap) and a compile of the emitted text. The stub runs in a PAGE, so it is assembled with
// the browser target: no `require` in its preamble, and a non-relative require is refused at bundle time. The ONE
// guard that stays is the caller's check for a CLOSING SCRIPT TAG — this payload is inlined into an HTML script
// element, and a closing tag inside it still cuts the fixture in half. (That guard caught this very comment: the
// first version of it spelled the sequence out, and the emitter refused to emit.)
//
// NO BACKTICKS ARE FORBIDDEN HERE ANY MORE, which is the point of the move: this is ordinary code.
// THE NAME IS DELIBERATELY NOT `P`: this stub declares its own `var P` for the query flags (see below), which
// SHADOWED this binding and made every fixture value undefined — the page then rendered the ids where labels
...
(function(){
  ...
  var SID = FIXTURE.sid, SESSION = FIXTURE.session, EVENTS = FIXTURE.events;
  ...
  // agent/scripts/harness-boot-check.mjs before believing any number.
    ...
    // THE BACKSLASHES HERE ARE ORDINARY NOW, AND THIS COMMENT USED TO WARN THAT THEY WERE NOT. While this stub was a
    // template literal, this line had to be written with a DOUBLED backslash — a single one collapsed to `/` and made
    // the emitted regex `/^.*/api//`, "Invalid regular expression flags", which killed the WHOLE stub (no token, no
    // sessions, no counter) and looked from the outside like a broken product. Round 270 moved the stub into this
    // module, where the text IS the code, so the plain form is the correct one — and the migration's line-verbatim
    // check against the old artifact is what proved these escape-residue lines were the only ones that changed.
