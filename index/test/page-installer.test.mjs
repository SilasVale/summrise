...
// `SummriseAgent-Setup.exe` carried etag `f1dc1c8e86ab125f9f8d078b23ed400c` — byte
// for byte the same file as `SummriseAgent-Setup-1.2.361.exe` — while the current
...
const page = (setupUrl) =>
  PAGE(
    ...
    "https://cdn.example/summrise-agent/summrise-agent-latest.tgz",
    setupUrl,
  );

test("an advertised installer is linked", () => {
  const html = page("https://cdn.example/summrise-agent/SummriseAgent-Setup.exe");
  assert.match(html, /Download Windows installer/);
  assert.match(
    html,
    /href="https:\/\/cdn\.example\/summrise-agent\/SummriseAgent-Setup\.exe"/,
